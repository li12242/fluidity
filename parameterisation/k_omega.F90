...
implicit none
  ...
  !  - wall functions are added to selected boundaries in komega_bcs and wall_functions

contains

subroutine komega_advdif_diagnostics(state)

  type(state_type), intent(inout) :: state
  
  call komega_damping_functions(state, advdif=.true.)  !! leave as is
  call komega_eddyvisc(state, advdif=.true.)	     !! eddy viscosity
  call komega_diffusion(state) 			     !! diff coeff
  call komega_tracer_diffusion(state)		     !! 3rd equation. What is this?
  call komega_calculate_rhs(state)		     !! source/sink terms on RHS of adv-diff eqs

end subroutine komega_advdif_diagnostics

subroutine komega_momentum_diagnostics(state)	     !! Reynolds Stress Tensor?	

  type(state_type), intent(inout) :: state
  
  call komega_damping_functions(state, advdif=.false.) !! leave as is
  call komega_eddyvisc(state, advdif=.false.)	     !! eddy viscosity

end subroutine komega_momentum_diagnostics

!--------------------------------------------------------------------------------!

subroutine komega_damping_functions(state, advdif) !!

  type(state_type), intent(in) :: state
  logical, intent(in) :: advdif
  
  type(scalar_field), pointer :: f_1, f_2, f_mu, y, dummydensity, density !! change f1,... ?
  type(scalar_field) :: k, omega !!
  type(tensor_field), pointer :: bg_visc
  integer :: node, stat
  real :: f_mu_val, f_1_val, f_2_val, Re_T, R_y, fields_max !! change f1,... ?
  character(len=FIELD_NAME_LEN) :: equation_type
  character(len=OPTION_PATH_LEN) :: option_path

  ewrite(1,*) 'in komega_damping_functions'
!  ewrite(1,*) "Amin has a go" !Amin!
  option_path = trim(state%option_path)//'/subgridscale_parameterisations/k-omega/' !! what is this?

  f_1 => extract_scalar_field(state, "f_1") !! change f1,... ?
  f_2 => extract_scalar_field(state, "f_2")
  f_mu => extract_scalar_field(state, "f_mu")

  ! initialise low_Re damping functions
  call set(f_1, 1.0) !! change f1,... ?
  call set(f_2, 1.0)
  call set(f_mu, 1.0)

  call get_option(trim(state%option_path)// &
       & "/subgridscale_parameterisations/k-omega/max_damping_value", fields_max) !!
  ...
  if (low_Re .or. &
     ...
     node_loop: do node = 1, node_count(k)
        ...
        if (node_val(bg_visc,1,1,node) <= fields_min) then
           ...
           f_1_val = 1.0
           f_2_val = 1.0
        else if (node_val(omega,node) <= fields_min) then !!
           ...
           Re_T = (node_val(density,node) * node_val(k,node)**2.0) / &
                (node_val(omega,node) * node_val(bg_visc,1,1,node)) !!
           R_y = (node_val(density,node) * node_val(k,node)**0.5 * node_val(y,node)) / &
                node_val(bg_visc,1,1,node)
...
end subroutine komega_damping_functions
...
subroutine komega_calculate_rhs(state)
  ...
  type(scalar_field), dimension(2) :: src_abs_terms !!! 3 to 2
  type(scalar_field), dimension(2) :: fields
  type(scalar_field), pointer :: src, abs, f_1, f_2, debug !! change f1,... ?
  ...
  ewrite(1,*) 'is this right? alpha, beta, beta_star !Amin!'
  ewrite(1,*) alpha
  ewrite(1,*) beta
  ewrite(1,*) beta_star
  ...
  call get_option('/physical_parameters/gravity/magnitude', g_magnitude, stat) !!! interesting
!!!  if (stat /= 0) then
!!!     have_buoyancy_turbulence = .false. !!! interesting
!!!  end if
...
!!!  end if
  ...
  !! whats going on? start with k and then do omega!
  field_loop: do i = 1, 2
     if (have_option(trim(option_path)//'scalar_field::'// &
          trim(field_names(i))//'/prescribed')) then !! for debugging purposes
        cycle
     end if
...
!!!     call allocate(src_abs_terms(3), fields(1)%mesh, name="buoyancy_term")
     ...
     do ele = 1, ele_count(fields(1))
        call assemble_rhs_ele(src_abs_terms, fields(i), fields(3-i), scalar_eddy_visc, u, & !! main function call
             ...
             f_1, f_2, ele, i, alpha, beta, beta_star) !!! c_eps_1, c_eps_2, sigma_p, buoyancy_density, have_buoyancy_turbulence removed
     end do
...
subroutine assemble_rhs_ele(src_abs_terms, k, omega, scalar_eddy_visc, u, density, & !!
     g, g_magnitude, &
     X, f_1, f_2, ele, field_id, alpha, beta, beta_star) !! change coeffs ? !!! c_eps_1, c_eps_2, sigma_p, have_buoyancy_turbulence, buoyancy_density removed
...
!!!  real, dimension(ele_ngi(u, ele)) :: scalar, c_eps_3 !!! scalar was used in Calculate buoyancy turbulence term
!!!  type(element_type), pointer :: shape_density !!! used in Calculate buoyancy turbulence term
!!!  real, dimension(:, :, :), allocatable :: dshape_density !!! used in Calculate buoyancy turbulence term
  ...
  do gi = 1, ngi
     reynolds_stress(:,:,gi) = scalar_eddy_visc_ele(gi)*(grad_u(:,:,gi) + transpose(grad_u(:,:,gi))) !! change? No
  end do
  do i = 1, dim
     reynolds_stress(i,i,:) = reynolds_stress(i,i,:) - (2./3.)*k_ele*ele_val_at_quad(density, ele) !! change? No
  end do
  ...
  if (field_id==2) then !! id=2 , hence applies to omega field and not k
     rhs = rhs*alpha*ele_val_at_quad(f_1,ele)*(omega_ele/k_ele) !! f1 is damping function
  end if
  rhs_addto(1,:) = shape_rhs(shape, detwei*rhs) !! shape_rhs ?

  ! A: !! Absorption ?
  rhs = -1.0*omega_ele*k_ele*beta_star*ele_val_at_quad(density, ele) !! changed! (k eq coeff)
  if (field_id==2) then
     rhs = rhs*(beta/beta_star)*ele_val_at_quad(f_2,ele)*(omega_ele/k_ele) !! chanegd (omega eq coeff)
  end if
  rhs_addto(2,:) = shape_rhs(shape, detwei*rhs)

  ! Gk:  
  ! Calculate buoyancy turbulence term and add to addto array
  ! No buoyancy term, so set this part of the array to zero. !! Currently no buoyancy !!!
!!!  rhs_addto(3,:) = 0.0    
...
subroutine komega_eddyvisc(state, advdif)
  ...
  type(scalar_field), pointer      :: scalar_eddy_visc, ll, f_mu, density, dummydensity !! uses f_mu
  type(scalar_field)               :: ev_rhs
  integer                          :: i, j, ele, stat
  
  ! Options grabbed from the options tree
!!  real                             :: c_mu !! no coeff needed since in k-omega eddyvis = rho*k/omega
  character(len=OPTION_PATH_LEN)   :: option_path
  ...
  ! Get model constant
!!  call get_option(trim(option_path)//'/C_mu', c_mu, default = 0.09) !! change
  ...
  ewrite_minmax(kk) !! what is ewrite !!??? writes to log file
  ewrite_minmax(omega) !!
  ...
  call allocate(ev_rhs, scalar_eddy_visc%mesh, name="EVRHS") !! eddy visc rhs ?
  call zero(ev_rhs)
  ...
  do i = 1, node_count(scalar_eddy_visc)
     call set(ll, i, max(node_val(kk,i), fields_min)**0.5 / max(node_val(omega,i), fields_min)) !! beta_star ? !!
  end do

  ! Calculate scalar eddy viscosity by integration over element
  do ele = 1, ele_count(scalar_eddy_visc)
     call komega_eddyvisc_ele(ele, X, kk, omega, scalar_eddy_visc, f_mu, density, ev_rhs) !! main function call
  end do
  ...
  contains
   ...
   subroutine komega_eddyvisc_ele(ele, X, kk, omega, scalar_eddy_visc, f_mu, density, ev_rhs) !!
   
      type(vector_field), intent(in)   :: x
      type(scalar_field), intent(in)   :: kk, omega, scalar_eddy_visc, f_mu, density !!
      ...
      rhs_addto = shape_rhs(shape_ev, detwei*ele_val_at_quad(density,ele)*&
                     ele_val_at_quad(f_mu,ele)*(kk_at_quad/omega_at_quad)) !! keep f_mu ? Yes
...
!  if (have_option(trim(option_path)//"/scalar_field::TurbulentKineticEnergy/prognostic")) then !A!
!    if (.not.have_option(trim(option_path)//"/scalar_field::TurbulentKineticEnergy"//& !A!
!         &"/prognostic/tensor_field::Diffusivity")) then !A!
!      ewrite(1,*) 'NoDiffusivity' !A!
!    else !A!
  ...
  ewrite(1,*) 'is this right? sigma and sigma_star !Amin!'
  ewrite(1,*) sigma
  ewrite(1,*) sigma_star
  ...
  do i = 1, node_count(diff)
     do j = 1, diff%dim(1)
        ...
        call addto(diff, j, j, i, node_val(eddy_visc, j, j, i) * sigma) !! changed
     end do
  end do
...
subroutine komega_tracer_diffusion(state)
  ...
  real                              :: local_background_diffusivity !!!, sigma_p, !! sigma p => buoyancy?
  ...
  do i_field = 1, scalar_field_count(state)
     ...
     if (have_option(trim(s_field%option_path)//&
        ...
        !!!   call addto(t_field, i, i, scalar_eddy_viscosity, 1.0/sigma_p) !! change?
        !!!end do
...
subroutine komega_bcs(state)
  ...
  type(state_type), intent(in)               :: state
  type(scalar_field), pointer                :: field1, field2    ! k or epsilon(omega) !!
  ...
  ! Depending on the equation type, extract the density or set it to some dummy field allocated above
  call get_option(trim(u%option_path)//"/prognostic/equation[0]/name", equation_type, stat=stat) !!!
  if (stat /= 0) then
    density=>dummydensity
  else
    select case(equation_type)
  ...
  ewrite(1,*) 'is this right? c_mu !Amin!'
  ewrite(1,*) c_mu
  ...
  field_loop: do index=1,2
     ...
     boundary_conditions: do i=0, nbcs-1
        ...
        call get_option(trim(bc_path_i)//"/type::k_omega/", wall_fns, stat=stat) !!

        if (trim(bc_type)=="k_omega" .and. wall_fns=="low_Re") then !!
           
           ! lowRe BC's are just zero Dirichlet or Neumann - damping functions get calculated in 
           ! komega_calc_rhs
           low_Re = .true.

        else if (trim(bc_type)=="k_omega" .and. wall_fns=="high_Re") then
           ...
           ewrite(2,*) "Calculating highRe k-omega BC: ",trim(field1%name),' ',trim(bc_name),' ',trim(bc_type),' ',trim(wall_fns) !!
           ...
           do sele = 1, ele_count(surface_mesh)
              call komega_wall_function(field1, X, u, masslump, scalar_eddy_visc, density, sele, & !!
                   index, c_mu, rhs_field) !! uses c_mu
           end do
...
!--------------------------------------------------------------------------------!
subroutine komega_wall_function(field1,X,U,masslump,scalar_eddy_visc,density,sele,index,c_mu,rhs_field)
  ...
  if (index==1) then
    ! k = tau_wall/rho/c_mu**0.5 !! change ?
    ...
    kappa = 0.43
    ! epsilon = (tau_wall/rho)**1.5/kappa/h !! change ?
...
end subroutine komega_wall_function
...
subroutine k_omega_check_options
  ...
  do istate = 0, n_phases-1
     ...
     if(.not. kmsh==emsh .or. .not. kmsh==vmsh .or. .not. emsh==vmsh) then
        FLExit("You must use the Velocity mesh for TurbulentKineticEnergy and TurbulentFrequency fields") 
     end if
